...
<!-- How is this related to agreement? -->
...
<!-- (Feynman doesn't give the original Japanese sentences, so I can just guess which strategies they were using.) -->

"How is this related to addressee? Isn't there another term for this, like [register](https://en.wikipedia.org/wiki/Register_(sociolinguistics))?"
...
Save your complaints to the third post, where I'm going to claim that preposition contraction is verbal agreement.
...
<!-- However, if the difference is morphological, then a good rule of thumb[^1] is to see them as one verb, and put all forms in the same inflection table. -->
...
<!-- 1. Politeness is determined by combinations of words and other constructors in the tree. For instance, if the sentence contains a polite word, such as `youPol_Pron` or `emperor_N`, we choose polite verb morphology. -->
<!-- <\!-- Extremely simplified example rules: -\-> -->
...
<!-- However, that's not enough to say that Japanese as a whole has person agreement.
Contrast with English, where _I wants_ is wrong in all contexts. -->
...
<!-- First, we need to distinguish between two _sources_ of politeness: -->

<!-- 1. __Subject-driven__---whether the subject is `i_Pron` or not-.param `Speaker` in  `VP`, actual forms for only a few verbs. -->
<!-- 2. __Situation-driven__---param `Style` in `VP`, actual forms for all verbs. -->

<!-- (Addressee counts as "situation-driven politeness", because there's nothing in the tree that reveals who you're talking to.) -->
...
Just to be clear: I don't think RGL should support _glance at_ and _observe_ as inflection forms of the same verb. A resource grammar should offer an API to syntactic structures and lexicon, not try to be too clever about usage.
...
<!-- We've just seen that the RGL `VP` contains inflection forms of the same verb (_mitai desu_, _mitagatte iru_, _mitagatte imasu_). -->
<!-- The inflection table is of type `Speaker => Style => … => Str`, and for most verbs, the `Speaker` doesn't actually make a difference. -->
<!-- Technically it's possible to include completely different verbs in the table (_glance at_ vs. _observe_), but I wouldn't recommend it, because there are constructions (like _wanting_ to do something) that use the full -->

#### Choice of verb form

<!-- As we've seen, the Japanese RG makes a difference for speaker in a few select constructions, like wanting and giving. If those are the only _speaker-related_ politeness distinctions you need, then the RGL category `VP` is enough. -->
...
__Style__---The `Style` parameter is open all the way up to `Utt`---finally, `PhrUtt` in  [PhraseJpn](https://github.com/GrammaticalFramework/gf-rgl/blob/master/src/japanese/PhraseJpn.gf#L8) chooses the respectful style.
...
<!-- Like all Extra and Extend functions, it is not exported in [RGL API](http://www.grammaticalframework.org/lib/doc/synopsis/index.html). -->
To use it in your grammar, you need to open ExtraJpn in your concrete syntax and use it like this:
...
<!-- [^6]: _"Okay, I agree that current actual Japanese doesn't have person agreement. But let's suppose that referring to yourself politely were always socially inappropriate, and anyone who says that must be a foreigner or a total weirdo. Would you then call it person agreement?"_<br/><br/>If a sociolinguistic rule creates gaps in the possible combinations of subject and verb form---literally nobody ever mixes the 3rd person and plain form, or 1st person and polite form, consistently with all verbs and constructions---then in time it can grammaticalise into actual person agreement. As a further development, the formerly offensive deviations might even become neutral, like they are in English: <em>I&nbsp;does</em> just says I don't speak the language well.<br/><br/>_"There's nothing special about being offended about language use. Many people get irrationally angry about English grammar, yet we don't consider split infinitives sociolinguistics."_<br/><br/>Look, you're going to like my 3rd post, where I'm going to claim that preposition contraction is person agreement. Now stop trolling in my footnotes and let the poor reader return to the main post. -->

<!--
If it's always inappropriate to talk about yourself politely---with all verbs and constructions, not just wanting in particular---that's stronger evidence for person agreement.

Wanting is actually a bad example, because polite speech in Japanese tries to avoid using wanting altogether. So you may be right about _mitagatte imasu_, that it's always weird with 1st person, but that weirdness isn't consistent with all possible verbs in Japanese. So I still think that actual Japanese in 2020 doesn't have pereson agreement.

We can still ask whether people would get confused as to who is the subject, or rather feel awkward due to a social rule violation. Contrast with a language that actually has person agreement. If I switched the subjects in English, the sentences would turn out as _*I wants […]_ and _*she want […]_. Most native speakers would argue that those are wrong on a purely grammatical level, not related to any social context.<br/><br/>_"I just said that only foreigners would ever say 'watashi wa … mitagatte imasu' in my Japanese. Now you define that English has person agreement, because saying 'I wants' makes you sound like a foreigner?"_<br/><br/>I agree, sounding like a foreigner is a valuable clue for deeming something grammatically incorrect. -->
