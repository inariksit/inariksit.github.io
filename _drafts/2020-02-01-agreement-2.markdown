...
- [Politeness in Japanese](#politeness-in-japanese)
    - [Implementation in the RGL](#implementation-in-the-rgl)
...
However, if the difference is morphological, then a good rule of thumb[^1] is to see them as one verb, and put all forms in the same inflection table. Here I have 3 levels, plain, respectful and extra respectful.
...
The `Style` parameter is open all the way up to `Utt`---finally, `PhrUtt` in  [PhraseJpn](https://github.com/GrammaticalFramework/gf-rgl/blob/master/src/japanese/PhraseJpn.gf#L8) chooses the respectful style.
