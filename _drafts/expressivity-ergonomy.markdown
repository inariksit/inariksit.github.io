...
![Handwave](/images/handwavy-explanation-chomsky-hierarchy.png "Simplified summary of Chomsky hierarchy and language complexity. Image source TODOLinkWikipedia")

TODO fix image:
- make the "weird mathematical stuff…" slide overlap with context-sensitive AND recursively enumerable
- attribute source of the image
- add a zoomed in version of the line between CF and CS, more lines
...
Writing a GF grammar for a non-context-free language like a^n^b^n^c^n^ is trivial, thanks to the lincats being *tuples of strings* instead of just strings.
...
```
and while in GF shell, you can print out the PGF with the command `pg` and observe the productions and sequences. But this is not at all necessary to follow the rest of this post—I'm just leaving the option for anyone who might be interested in exploring more. And if you just now got interested in formal language theory and GF, here's the link to [Ljunglöf (2004)](https://gupea.ub.gu.se/bitstream/handle/2077/16377/gupea_2077_16377_3.pdf) again.
-->
...
<!-- Coming from GF-land, it shocks me what kind of things people call ASTs. I see people writing grammars where *inflection forms of the same lexeme are in different functions*. 🙀 -->
...
The properties of GF that made it *possible* to implement a^n^b^n^c^n^ are the same that make it *pretty* to implement this grammar. Let's discuss them one at a time.
