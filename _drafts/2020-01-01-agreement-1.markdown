...
Back to the regularly scheduled program! We're ready to move on to 2-dimensional agreement. There are, in fact, much fewer different ways of doing it, because we must include both. I will show one nominative-accusative language, and one ergative-absolutive language, but really, if we consider just the implementation of `V`, `V2`, `VP`, `UseV`, `ComplV2` and `PredVP`, there's not all that much difference.
