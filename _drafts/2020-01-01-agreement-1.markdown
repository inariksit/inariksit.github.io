...
## Interlude: Nominative-accusative vs. Ergative-absolutive
...
<ul>
 <li> Inarik <font color="purple">ni</font> nau  <br/>
 ...
 * Inari  da  
   *Inari is*
<ul>
<li><font color="green">Nik</font> Inari dut <br/>
   <em><font color="green">I</font> have Inari</em>
</li>
</ul>

Pretty solid ergative-absolutive alignment from the NPs! Notice also how the verb indexes the absolutive argument (S or O): <b>n</b>aiz and <b>n</b>au for 1st person singular, <b>d</b>a and <b>d</b>ut for 3rd person singular. If you just looked at the translations of "Inari is" and "Inari has me", you wouldn't find any similarities in the verb forms, *da* and *nau*.

Languages that behave like this are called "ergative-absolutive languages", or "absolutive-ergative languages". (Some say that to have a consistent naming scheme with nominative-accusative, absolutive should come first because it's the unmarked case.) However, everyone agrees on the short name, "ergative languages"---again, named after the marked case.
...
As you've seen, these terms conflate features from nominal morphology, verbal person marking and even syntax, whereas in the real world, they don't *need* to appear in just those combinations. For example, Somali is a nominative-accusative language with marked nominative, and unmarked accusative. Indonesian has a weird word order.
...
I don't know about you but I'm tired of pretending English has morphology, so we're graduating from Like-English-but-… to actual languages.
...
Here's a grammar of very simplified Hungarian---just enough morphology to illustrate how to deal with 2-dimensional agreement. There are lots and lots of cases,
Hungarian is nominative-accusative.


```haskell
param
  AgrS = Sg1 | Sg2 | Sg3 | Pl1 | Pl2 | Pl3 ;
  AgrO = DefObj | IndefObj ;
  Case = Nom | Acc | … ; -- Lots of cases we're not showing here!

lincat
  V,
  VP = {s :         AgrS => Str} ;
  V2 = {s : AgrO => AgrS => Str} ;
  NP = {s : Case => Str ; a : AgrS} ;
...
* WALS article about alignment of person marking: [wals.info/chapter/102](https://wals.info/chapter/100)
