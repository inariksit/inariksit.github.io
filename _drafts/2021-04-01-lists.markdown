...
While the example is for `NP` (noun phrase), exactly the same principle holds for other list categories in the RGL ([AP](http://www.grammaticalframework.org/lib/doc/synopsis/index.html#ListAP), [RS](http://www.grammaticalframework.org/lib/doc/synopsis/index.html#ListRS), TODO rest).
...
cat [NP]{2}
```
<em><small>We can even verify that this was the expression: look at the [RGL abstract syntax](TODO)!</small></em>
...
And here's what we do in `BaseAP` and `ConsAP`. We need to choose the `AConj` form of all of the `AP`s except for the last one.
...
lin
  BaseAP a1 a2 = {
    s1 = \\_ => a1.s ! AConj ; -- Always conj. form
    s2 = a2.s    -- Retain full inflection table
    }

  ConsAP a as = as ** {
    s1 = \\x => a.s ! AConj ++ -- Always conj. form
                "," ++         -- Any separator you want
                as.s1 ! x      -- (already in AConj)
    } ;

```

We need to have the `AConj` form in `AP`, because any `AP` can potentially appear before a conjunction---even an `AP` that is already built out of several. (*The house is [big and blue] or [small and green]*.) But if the `AP` has ever been in a coordinated chain, then it won't ever go back to a non-conjunctive form.
...
Or to be a bit more interesting, we use a different version of the conjunctions depending on the part of speech.

```haskell
param
  ConjType = And POS | Or POS ;
  POS = Noun | Verb ;
```
