...
lincat
  ...
  Field = NP ;
  [Field] = {s : ListNP ;
             first : NP ; -- if only 1 Field in the list
             size : Size} ;
param
  Size = Empty | One | More ;

lin
  ClassDef name flds = let
    subj : NP = symb name ;
    pred = mkVP (
      case attrs.size of {
        Empty
          => mkNP (mkPN "a class with no fields") ;
        One
          => mkNP (mkCN (mkN2 "a class with the field")
                        attrs.first) ;
        More
          => mkNP (mkCN (mkN2 "a class with the fields")
                        (mkNP and_Conj attrs.s))
          }) ;
