...
Let me demonstrate variants on a slightly more complex example. Since it's really hard to get Foods to blow up,
I need to change the abstract syntax a bit. We are going to make the determiner undergo free variation: no matter if we type "this pizza is vegan" or "these pizzas are vegan", we get the tree `Pred Pizza Vegan`.
...
Actually, while we're at it, let's ignore all the restrictions of agreement.
...
This code accepts "these pizzas is", "this pizza are", "these pizza (is|are)", "this pizzas (is|are)".

Now, let's compare the original, where we have `Pred` applied to `Item`. These are the sequences used in `Pred`.
...
We get 64 sequences for the original 4.

You might argue that this is an extraordinarily stupid way to write variants in a Foods grammar. You're actually right, that was like the third try, because the grammar just didn't blow up enough with my earlier attempts.

Here's how to do variants in the intended way.

```haskell
TODO: code
```

Plot twists: the version with variants has *fewer* sequences.
...
It's not surprising, because the new abstract syntax has fever funs: `This`, `That`, `These` and `Those` don't exist in the new abstract, they're just variants in `Pred`. But if we just look at the sequences, technically there's now 4 sequences fewer.
...
There are still more productions and concrete functions, __and__ we've still lost the distinction between all determiners, so I don't think we've won anything with this. Just thought it was a funny detail.
