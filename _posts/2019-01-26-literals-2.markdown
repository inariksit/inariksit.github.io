...
`NItemsLiteral` is dangerous, because one day someone might change the lincat of
...
previously. In fact, the code is identical, except that now we choose
another `mkNP` instance for `item`.
...
You might wonder, why not make the `String` literal into a `Det` and
call a proper `mkNP` instance for `sym` (which is now Det) and the
`kind`? Well, we can of course do that: create a new `Det` where we
insert the string from the `String` in the right field. The
linearisation for `LiteralKind` would look like following, with ???
...
*But what if there is some `mkDet` constructor in
 ParadigmsEng or ResEng, can't we use that?* Let's examine
 ...
 argument to a function. So we can't use `mkDeterminerSpec`, because that would give us [unsupported token gluing](https://gist.github.com/inariksit/edde72f43d439571c79f8ef758443c11) exception.
...
```haskell
LiteralKind string kind =
  let symDet : Det = a_Det ** {s = string.s} ;
      -- symDet : Det = a_Det ** string ; -- works equally well
      symItem : NP = mkNP symDet kind ;
   in mkS (mkCl symItem) ;
```
...
There's one more gotcha though: we can't *parse* literals that
...
to all your problems you didn't knew you had. Some mornings I read
